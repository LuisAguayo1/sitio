...
public function __construct($config=""){
  ...
  'currency' => 'MXN',                       //paypal currency
  'cursymbol'=> '$',                   //currency symbol
  'location' => 'MX',                        //location code  (ex GB)
  'returnurl'=> 'http://mysite/myreturnpage',//where to go back when the transaction is done.
  'returntxt'=> 'Finalizar compra',         //What is written on the return button in paypal
       ...
       if(!empty($this->items)){foreach($this->items as $item){
         $form.='
          <div id="item_'.$cpt.'" class="itemwrap">
            ...
            <input type="hidden" name="quantity_'.$cpt.'" value="'.($item['quantity']*1).'" />
            <input type="hidden" name="amount_'.$cpt.'" value="'.($item['price']*1).'" />
           <input type="hidden" name="shipping_'.$cpt.'" value="'.($item['shipping']*1).'" />
         </div>';
         $cpt++;
       }}
